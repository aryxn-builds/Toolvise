...
import { trackStackView } from "@/lib/analytics"
...
// ── Export helpers ─────────────────────────────────────────────────────────
function exportAsJSON(data: StackResult) {
  const blob = new Blob([JSON.stringify(data, null, 2)], { type: "application/json" })
  const url = URL.createObjectURL(blob)
  const a = document.createElement("a")
  a.href = url
  a.download = `toolvise-stack-${Date.now()}.json`
  a.click()
  URL.revokeObjectURL(url)
}

function exportAsMarkdown(data: StackResult) {
  const lines: string[] = []
  lines.push(`# Toolvise Stack Report`)
  lines.push("")
  if (data.formInput?.description) {
    lines.push(`> **Project:** ${data.formInput.description}`)
    lines.push("")
  }
  lines.push(`## Summary`)
  ...
  lines.push("")
  lines.push(`## Tools`)
  lines.push("");  lines.push(`| Tool | Category | Free | Reason |`)
  lines.push(`|------|----------|------|--------|`)
  for (const t of data.tools || []) {
    lines.push(`| ${t.name} | ${t.category} | ${t.isFree ? "✅" : "💳"} | ${(t.reason || "").replace(/\|/g, " ")} |`)
  }
  lines.push("")
  if (data.roadmap?.length) {
    lines.push(`## Roadmap`)
    for (const step of data.roadmap) lines.push(`- ${step}`)
    lines.push("")
  }
  if (data.proTip) {
    lines.push(`## Pro Tip`)
    lines.push(data.proTip)
    lines.push("")
  }
  lines.push(`---`)
  lines.push(`*Generated by [Toolvise](https://toolvise.vercel.app)*`)

  const blob = new Blob([lines.join("\n")], { type: "text/markdown" })
  const url = URL.createObjectURL(blob)
  const a = document.createElement("a")
  a.href = url
  a.download = `toolvise-stack-${Date.now()}.md`
  a.click()
  URL.revokeObjectURL(url)
}
      ...
      if (slug) {
        try {
          ...
          if (dbData && !error) {
            ...
            trackStackView(dbData.id, viewer?.id ?? null)
      ...
      <header className="animate-in fade-in slide-in-from-top-4 duration-300">
        <div className="flex flex-col gap-6 md:flex-row md:items-start md:justify-between">
          ...
          <div className="flex shrink-0 flex-wrap items-center justify-end gap-2">
            ...
            {data.shareSlug && (
              <Link
                href={`/advisor?remix=${data.shareSlug}`}
                className="flex items-center gap-2 rounded-full border border-[rgba(240,246,252,0.10)] bg-[#161B22] px-5 py-2.5 text-sm font-semibold text-[#E6EDF3]/80 transition-all hover:bg-[#0D1117] hover:text-[#E6EDF3]"
              >
                <GitFork className="h-4 w-4 text-[#1ABC9C]" />
                Remix
              </Link>
            )}
            ...
            <div className="relative" ref={exportRef}>
              <button
                ...
                className="flex items-center gap-2 rounded-full border border-[rgba(240,246,252,0.10)] bg-[#161B22] px-5 py-2.5 text-sm font-semibold text-[#E6EDF3]/80 transition-all hover:bg-[#0D1117] hover:text-[#E6EDF3]"
              >
                <Download className="h-4 w-4 text-[#2EA043]" />
                Export
                <ChevronDown className={cn("h-3.5 w-3.5 transition-transform", exportOpen && "rotate-180")} />
              </button>
              {exportOpen && (
                <div className="absolute right-0 top-full mt-2 w-44 rounded-2xl border border-[rgba(240,246,252,0.10)] bg-[#161B22] shadow-glass py-1 z-30 animate-in fade-in zoom-in-95 duration-150">
                  <button
                    onClick={() => { exportAsJSON(data); setExportOpen(false) }}
                    className="w-full flex items-center gap-3 px-4 py-2.5 text-sm text-[#E6EDF3]/70 hover:bg-[#0D1117] hover:text-[#E6EDF3] transition-colors"
                  >
                    <FileJson className="h-4 w-4 text-[#2EA043]" />
                    Export JSON
                  </button>
                  <button
                    onClick={() => { exportAsMarkdown(data); setExportOpen(false) }}
                    className="w-full flex items-center gap-3 px-4 py-2.5 text-sm text-[#E6EDF3]/70 hover:bg-[#0D1117] hover:text-[#E6EDF3] transition-colors"
                  >
                    <FileText className="h-4 w-4 text-[#1ABC9C]" />
                    Export Markdown
                  </button>
                </div>
              )}
