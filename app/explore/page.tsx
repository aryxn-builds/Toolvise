...
export default function ExplorePage() {
  ...
  return (
    <div className="min-h-dvh bg-[#0D1117] text-[#E6EDF3] selection:bg-[#161B22]/2 relative pb-24">
      <Navbar />
      ...
      <main className="relative mx-auto w-full max-w-7xl px-4 py-16 sm:px-6 lg:px-8">
        ...
        <header className="mb-14 text-center space-y-4 animate-in fade-in slide-in-from-top-4 duration-300">
          <Badge variant="outline" className="border-[#2EA043]/25 bg-[#0D1117]/10 text-[#8B949E] px-4 py-1.5 text-sm">
            <Layers className="mr-2 h-4 w-4" />
            {totalCount.toLocaleString()} Generated Stacks
          </Badge>
          <h1 className="bg-gradient-to-br from-[#2EA043] to-[#1ABC9C] bg-clip-text text-4xl font-extrabold tracking-tight text-transparent sm:text-6xl">
            Explore Stacks
          </h1>
          <p className="text-lg text-[#E6EDF3]/50 max-w-2xl mx-auto font-medium">
        ...
        <div className="mb-12 space-y-6 flex flex-col items-center animate-in fade-in zoom-in-95 duration-300 delay-75 fill-mode-both">
          <div className="relative w-full max-w-2xl">
            <Search className="absolute left-4 top-3.5 h-5 w-5 text-[#E6EDF3]/40" />
            <Input
              type="text"
              placeholder="Search by tech, idea, or tags... (e.g. Next.js, AI CRM)"
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              className="input-dark h-12 w-full pl-12 text-base"
            />
          </div>
          ...
          <div className="flex flex-wrap items-center justify-center gap-2 max-w-3xl">
            {FILTERS.map(filter => (
              <Button
                ...
                className={cn(
                  "rounded-full px-5 transition-all text-sm font-semibold",
                  activeFilter === filter 
                    ? "btn-primary shadow-[0_0_15px_rgba(46,160,67,0.4)]" 
                    : "btn-ghost"
                )}
              >
                {filter}
        ...
        {loading ? (
          <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
            {[1, 2, 3, 4, 5, 6, 7, 8].map(n => (
              <Card key={n} className="h-72 card-3d animate-pulse" />
            ))}
          </div>
        ) : stacks.length > 0 ? (
          <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 animate-in fade-in fill-mode-both duration-300 delay-100">
            {stacks.map((stack) => {
              ...
              return (
                <Card 
                  key={stack.id} 
                  className="group relative flex flex-col overflow-hidden card-3d h-[320px]"
                >
                  ...
                  <CardContent className="flex flex-1 flex-col p-6 h-full">
                    ...
                    {/* Description */}
                    <p className="text-sm font-medium text-[#E6EDF3]/90 leading-relaxed line-clamp-3 mb-4">
                      &quot;{stack.user_input}&quot;
                    </p>
                    ...
                    <div className="flex flex-wrap gap-2 mb-auto shrink-0">
                      {stack.tools?.slice(0, 3).map((t, idx) => (
                        <div key={idx} className="flex items-center gap-1.5 text-xs text-[#E6EDF3]/50 bg-[#0D1117] rounded-md px-2 py-1">
                      ...
                      {stack.tools?.length > 3 && (
                        <div className="text-xs text-[#E6EDF3]/40 bg-[#0D1117] rounded-md px-2 py-1 flex items-center">
                    ...
                    {/* Bottom Actions */}
                    <div className="flex items-center justify-between mt-6 pt-4 border-t border-[rgba(240,246,252,0.10)] shrink-0">
                        ...
                        <div className="relative group/upvote">
                          <button
                            ...
                            className={cn(
                              ...
                              hasVoted
                                ? "bg-[#0D1117]/20 text-[#2EA043]"
                                : "bg-[#0D1117] text-[#E6EDF3]/50 hover:bg-[#0D1117] hover:text-[#E6EDF3]"
                            )}
                          >
                          ...
                          {!isLoggedIn && (
                            <div className="absolute bottom-full left-0 mb-1.5 hidden group-hover/upvote:block whitespace-nowrap glass-strong px-2.5 py-1.5 text-xs shadow-lg">
                              Sign in to upvote
                            </div>
                          )}
                        </div>
                        ...
                        <button
                          ...
                          className={cn(
                            ...
                            bookmarked.has(stack.id)
                              ? "text-[#2EA043] font-semibold"
                              : "text-[#E6EDF3]/30 hover:text-[#2EA043]"
                          )}
                        >
          ...
          <div className="flex flex-col items-center justify-center py-20 text-center animate-in fade-in duration-500">
            <div className="h-20 w-20 rounded-full bg-[#0D1117] flex items-center justify-center mb-6">
              <Inbox className="h-8 w-8 text-[#E6EDF3]/30" />
            </div>
            <h3 className="text-xl font-bold text-[#E6EDF3] mb-2">No stacks found</h3>
            <p className="text-[#E6EDF3]/50 max-w-sm">
          ...
          <div className="mt-16 flex justify-center">
            {hasMore ? (
              <Button
                ...
                variant="outline"
                className="btn-ghost h-12 px-8 rounded-full gap-2 font-semibold"
              >
                {loadingMore && <Loader2 className="h-4 w-4 animate-spin text-[#2EA043]" />}
                {loadingMore ? "Loading..." : "Load More Stacks"}
              </Button>
            ) : (
              <p className="text-sm font-medium text-[#E6EDF3]/40 bg-[#0D1117] px-6 py-2 rounded-full border border-[rgba(240,246,252,0.10)] shadow-inner">
