    ...
    <div className="min-h-dvh bg-white text-[#111827] selection:bg-[#F97316]/30 relative pb-24">
      ...
      <main className="relative mx-auto w-full max-w-7xl px-4 py-16 sm:px-6 lg:px-8">
        ...
        <header className="mb-14 text-center space-y-4 animate-in fade-in slide-in-from-top-4 duration-700">
          ...
          <p className="text-lg text-[#111827]/50 max-w-2xl mx-auto font-medium">
        ...
        <div className="mb-12 space-y-6 flex flex-col items-center animate-in fade-in zoom-in-95 duration-700 delay-150 fill-mode-both">
          <div className="relative w-full max-w-2xl">
            <Search className="absolute left-4 top-3.5 h-5 w-5 text-[#111827]/40" />
            <Input
              type="text"
              placeholder="Search by tech, idea, or tags... (e.g. Next.js, AI CRM)"
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              className="h-12 w-full pl-12 rounded-xl bg-white border-[#FFD896] text-[#111827] placeholder:text-[#111827]/30 focus-visible:ring-1 focus-visible:ring-[#F97316] focus-visible:border-[#F97316] transition-all text-base"
            />
          </div>
          ...
          <div className="flex flex-wrap items-center justify-center gap-2 max-w-3xl">
            {FILTERS.map(filter => (
              <Button
                ...
                className={cn(
                  "rounded-full border-[#FFD896] px-5 transition-all text-sm font-semibold",
                  activeFilter === filter 
                    ? "bg-[#F97316] text-[#111827] border-[#F97316] shadow-[0_0_15px_rgba(249,115,22,0.4)]" 
                    : "bg-white text-[#111827]/60 hover:bg-white hover:text-[#111827]"
                )}
              >
                {filter}
              </Button>
        ...
        {loading ? (
          <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
            {[1, 2, 3, 4, 5, 6, 7, 8].map(n => (
              <Card key={n} className="h-72 bg-white border-[#FFD896] animate-pulse rounded-2xl" />
            ))}
          </div>
        ) : stacks.length > 0 ? (
          <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 animate-in fade-in fill-mode-both duration-700 delay-300">
            {stacks.map((stack) => {
              ...
              return (
                <Card 
                  key={stack.id} 
                  className="group relative flex flex-col overflow-hidden bg-white border-[#FFD896] hover:border-[#FFD896] hover:shadow-[0_8px_30px_rgba(0,0,0,0.5)] transition-all rounded-2xl h-[320px]"
                >
                  ...
                  <CardContent className="flex flex-1 flex-col p-6 h-full">
                    ...
                    <div className="flex items-start justify-between gap-2 mb-4 shrink-0">
                      <Badge className="bg-white text-[#111827]/80 border-0 uppercase tracking-wider text-[10px] font-bold">
                        {stack.skill_level}
                      </Badge>
                    ...
                    {/* Description */}
                    <p className="text-sm font-medium text-[#111827]/90 leading-relaxed line-clamp-3 mb-4">
                      &quot;{stack.user_input}&quot;
                    </p>
                    ...
                    <div className="flex flex-wrap gap-2 mb-auto shrink-0">
                      {stack.tools?.slice(0, 3).map((t, idx) => (
                        <div key={idx} className="flex items-center gap-1.5 text-xs text-[#111827]/50 bg-white rounded-md px-2 py-1">
                      ...
                      {stack.tools?.length > 3 && (
                        <div className="text-xs text-[#111827]/40 bg-white rounded-md px-2 py-1 flex items-center">
          ...
          <div className="flex flex-col items-center justify-center py-20 text-center animate-in fade-in duration-500">
            <div className="h-20 w-20 rounded-full bg-white flex items-center justify-center mb-6">
              <Inbox className="h-8 w-8 text-[#111827]/30" />
            </div>
            <h3 className="text-xl font-bold text-[#111827] mb-2">No stacks found</h3>
            <p className="text-[#111827]/50 max-w-sm">
          ...
          <div className="mt-16 flex justify-center">
            {hasMore ? (
              <Button
                ...
                variant="outline"
                className="h-12 px-8 rounded-full border-[#FFD896] bg-white text-[#111827] hover:bg-white gap-2 font-semibold"
              >
                {loadingMore && <Loader2 className="h-4 w-4 animate-spin text-[#F97316]" />}
                {loadingMore ? "Loading..." : "Load More Stacks"}
              </Button>
            ) : (
              <p className="text-sm font-medium text-[#111827]/40 bg-white px-6 py-2 rounded-full border border-[#FFD896] shadow-inner">
